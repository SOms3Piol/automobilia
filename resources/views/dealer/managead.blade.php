            ...
            <div class="grid grid-cols-2 max-md:grid-cols-1 gap-3 px-3 py-5">
                <div class="card bg-white shadow-md rounded-md px-3 py-2">
                    ...
                    <div class="flex gap-3 mt-3">
                        ...
                        @if ($vehicle->active)
                            <form class="block w-full" action="{{ route('disable.ad', $vehicle) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <input type="text" hidden value="{{ $vehicle->id }}" name="id">
                                <button class="bg-red-700 rounded font-semibold cursor-pointer py-3 text-white w-full" 
                                        type="submit">Disable</button>
                            </form>
                        @else
                            <form class="block w-full" action="{{ route('active.ad', $vehicle) }}" method="post">
                                @csrf
                                <input type="text" hidden value="{{ $vehicle->id }}" name="id">
                                <button class="bg-green-700 rounded font-semibold cursor-pointer py-3 text-white w-full" 
